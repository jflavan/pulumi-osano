...
- On a pull request, `Analyze (<language>)` fails when the pull request introduces a CodeQL alert of any severity, meaning an open alert that is not dismissed and not already open on `main`. The job annotates each new alert on the changed line and lists them in the job summary. It also fails when the analysis itself fails, for example during tool setup or upload, or when no Go module can be extracted. Pushes to `main` and scheduled runs never fail because of alerts; their alerts go to the Security and quality tab. If the job cannot read the code scanning API, as can happen with a fork's read-only token, it instead fails on any CodeQL result on the lines the pull request changes. That fallback cannot see dismissals.
- `CodeQL gate` fails when any `Analyze` job fails or is cancelled. A branch ruleset on `main` requires `CodeQL gate` and `Code scanning results / CodeQL`, so a pull request with a new CodeQL alert cannot be merged. The ruleset also blocks direct pushes of commits that have not passed these checks.
