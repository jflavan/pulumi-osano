...
- `Analyze (<language>)` fails only when the analysis itself fails, for example during tool setup or upload, or when no Go module can be extracted. Read the job log.
...
- To dismiss a false positive, go to **Security and quality > Code scanning** and give a reason. Inline suppression comments (`codeql[...]`, `lgtm`) have no effect in this setup for any language. For an alert in `sdk/`, fix the provider or schema and run `make codegen`. Never hand-edit generated files.
- Keep code scanning "default setup" disabled in the repository settings. Do not rename the workflow file, the `analyze` job, the matrix languages, or the `/language:<language>` categories. If one is renamed, run the workflow on `main` (**Actions > CodeQL Advanced > Run workflow**) and delete the old configuration under **Security and quality > Code scanning > Tool status**. Until then, pull requests show a neutral "configuration not found" result.
